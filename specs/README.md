...
that span **two or more** packages. Single-package specs live next to their package
...
| `e2e` only | `e2e/specs/` |
| **cross-module (≥ 2 packages)** | `specs/` (here) |

Note: `e2e/specs/` also contains the deterministic `*.flow.json` browser flows — SDD
specs (`*.md`) live alongside them.
...
Implementation Plan (`docs/plans/`). The intended chain is:

```
spec-creator → spec (WHAT/WHY) → implementation-planner → plan (HOW) → run-plan → code
```
