...
2. **Deep (the `deep` flag).** Parse the JSONL journals for exact, per-turn numbers
   ...
   - It prints per-agent and total tokens, **cache hit ratio**, tool calls, wall-clock span, and a
     **parallelism factor** (Σ agent spans ÷ wall-clock). Nested sub-agents (spawnDepth > 1) are
     ...
     `max_depth=`). For a cost estimate pass `--prices .claude/skills/workflow-retro/prices.json` —
     a bundled rate map covering the models this project's agents run on (opus-4-8, sonnet-5,
     haiku-4-5), carrying a `_verified` date. **Prices drift — re-confirm current per-model rates
     via the `claude-api` skill and refresh that file if `_verified` is stale** before trusting a
     cost figure; do not hard-code rates inline.
