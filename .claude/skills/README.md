...
| [run-plan](run-plan/SKILL.md) | Workflow | SDD executor: runs an approved Implementation Plan (`docs/plans/`) — implementers per DAG, architecture-reviewer + plan-verifier gate, bounded fix loop; no test-writer, never pushes |
| [workflow-retro](workflow-retro/SKILL.md) | Workflow | Post-mortem for a multi-agent run: metrics (tokens, cache efficiency, tool calls, parallelism, cost) + qualitative insights (hard/easy/duplicated/missed) + actionable recommendations; appends a trend row to `docs/retros/ledger.md`. Manual only, never auto-triggered |
