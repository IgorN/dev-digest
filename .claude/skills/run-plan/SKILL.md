...
description: >
  ...
  architecture-reviewer + plan-verifier in parallel (plus security-reviewer when the diff touches
  auth/input/secrets/endpoints), then resolves their comments in a bounded fix loop. Starts FROM a plan — spec authoring (spec-creator) and planning (implementation-planner) are
...
|-------|---------|---------|
| `plan:<path>` | Path to the approved Implementation Plan (usually `docs/plans/<x>.md`). **Required.** | — |
...
| `max-fix:<n>` | Cap on the review fix loop (Step 3). | `3` |
...
- **No test-writer.** A dedicated test-authoring pass is intentionally disabled for this command
  (token cost). Coverage comes only from each implementer's self-verification (the module's existing
  tests + typecheck). Do not spawn `test-writer`.
...
- **Orchestrator-owned tasks stay with you.** Tasks the plan marks `orchestrator/human` — e2e work
  and lock-step edits to the vendored `@devdigest/shared` copies (`server/src/vendor/shared` +
  `client/src/vendor/shared`) — are done in the main session or surfaced to the user, never handed
  to a parallel `implementer`.
...
Read the plan file. Extract for every task: `T-id`, `Action`, `Package`, `Type`, `Owner`,
`Skills to use`, `Owned paths`, `Depends-on`, `Known gotchas`, `Acceptance`. Read the plan's
`## Execution mode` field; a `mode:` arg overrides it. Build the dependency DAG from `Depends-on`.
...
1. Find the **ready set** — tasks whose `Depends-on` are all complete and whose `Owned paths` do not
   overlap any task already running this batch (skip `orchestrator/human` tasks — handle those
   yourself at the right DAG point).
...
Compute the **changed-file set**. Implementers do **not** commit, so a commit-range diff alone
misses their work — union all three, like `pr-self-review` does:
...
git diff --name-only                     # unstaged (the implementers' edits)
git diff --name-only --cached            # staged
```

Cross-check the union against the implementer reports. Then spawn, **concurrently**:

- **`architecture-reviewer`** on the changed-file set → structural findings (severity + rule) and a
  PASS/FAIL gate.
- **`plan-verifier`** with the plan + changed set → a traceability matrix (by T-id and, when the
  plan cites a spec, by `AC-N`) and a PASS/FAIL/REVIEW gate.
- **`security-reviewer`** — **conditionally**: only when the changed set touches security-relevant
  ground (auth/authz, user input handling, file uploads, secrets/`container.secrets`, new or changed
  API endpoints, raw SQL). It runs on Opus, so don't spawn it for a plain UI/refactor diff — state
  in the report whether it ran or was skipped and why.

Pass every reviewer the changed-file set explicitly — they have no `Bash` and cannot diff
themselves. `architecture-reviewer`/`plan-verifier` are pinned to Sonnet in their agent definitions
(read-only, structured prompts — the cheap gate). Collect the verdicts.

### Step 3 — Fix loop (bounded — this is where review comments get resolved)

Build the **fix backlog**:
- `architecture-reviewer` findings marked **blocking** (needs-discussion → report only).
- `plan-verifier` items that are **missing** or **partial** (a requirement is not actually met).
- `security-reviewer` findings (if it ran) with severity **Critical** or **High** (Medium/Low →
  report only). On re-review, re-run it scoped to the touched files only.
...
### Review gate
- architecture-reviewer (sonnet): PASS | FAIL — <blocking/needs-discussion counts>
- plan-verifier (sonnet): PASS | FAIL | REVIEW — <verified N/M; missing/partial ids>
- security-reviewer (opus): PASS | FAIL — <crit/high counts> | skipped — <why (no security-relevant files)>
