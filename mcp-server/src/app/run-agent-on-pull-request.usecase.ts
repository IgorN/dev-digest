...
export type RunAgentOnPullRequestOutput =
  | {
      run_id: string;
      status: 'done';
      verdict: string | null;
 ...
 */
export function makeRunAgentOnPullRequestUseCase(deps: {
  api: DevDigestApi;
  cache: RunPrCache;
  ...
  return async function runAgentOnPullRequest(
    input: RunAgentOnPullRequestInput,
  ): Promise<RunAgentOnPullRequestOutput> {
...
async function finalizeOutcome(
  ...
  if (status === 'failed' || status === 'cancelled') {
    return {
      ...
      next_action: 'Inspect the run in the DevDigest app, or call run_agent_on_pull_request again to retry.',
    };
  }
...
function mapTriggerError(err: unknown, input: RunAgentOnPullRequestInput): McpToolError {
