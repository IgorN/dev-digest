...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // All 4 sides fully longhand — `borderColor` (top+right+bottom+LEFT) would
    // still overlap `borderLeftColor` below and trip React's shorthand/longhand
    // rerender warning, even though neither is the `border` shorthand itself.
    borderStyle: "solid",
