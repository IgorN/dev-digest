/* ContextTab — attach/detach the active repo's context documents (markdown
   under specs/docs/insights) to an agent, and drag to reorder. Order is the
   order the documents are injected — as an untrusted "## Project context"
   block — into every run. Attached documents sort to the top (draggable); the
   rest follow. Every change persists the full ordered path set (mirrors
   SkillsTab / useSetAgentSkills). */
"use client";

import React from "react";
import { useTranslations } from "next-intl";
import { Badge, Checkbox, EmptyState, Icon, IconBtn, Markdown, Skeleton } from "@devdigest/ui";
import type { Agent, DocumentInventoryItem } from "@devdigest/shared";
...
export function ContextTab({ agent }: { agent: Agent }) {
  ...
  const setDocs = useSetAgentContextDocuments(agent.id);

  // Local ordered list of ATTACHED document paths, seeded from the agent.
  ...
  React.useEffect(() => {
    setAttached(agent.context_documents ?? []);
